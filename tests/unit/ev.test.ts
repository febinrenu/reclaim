 ...
 * WHATSAPP_NUDGE with no lift applied (pRecover === pBase === 0.71, isolating the EV
 * arithmetic from the scoring step), a ₹0.35 nudge cost, and a contact-fatigue term
 * from BUILD_PLAN.md §6.1 correction 2 on a ₹6,000 LTV customer with no prior contact
 * this week. This exact scenario is the one already exercised (without an assertion)
 * by tests/unit/purity.test.ts's `money` domain exercise — this test is where it
 * becomes a pinned number. Intended to be quoted directly in the README.
 */
describe('the hand-worked EV example', () => {
  it('matches an independently hand-computed total to the millipaise', () => {
    ...
    const breakdown = computeEv(
      ...
      { ...policy, liftLogit: { ...policy.liftLogit, WHATSAPP_NUDGE: 0 } },
    )
